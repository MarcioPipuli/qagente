...
Este agente é acompanhado por um arquivo de regras (`AGENTS.md`, com espelho em `CLAUDE.md`) e por 15 skills especializadas em `skills/` — as fases do fluxo, duas alternativas para a automação de UI, 1 skill de referência gramatical usada dentro da Fase 2, 8 skills de apoio que entram fora da sequência das fases, e 1 skill de configuração que preenche os dois arquivos de `.qagente/`. Leia `AGENTS.md` para os princípios e o fluxo de trabalho completos antes de iniciar qualquer tarefa não trivial — este arquivo é apenas o cartão de identidade do agente.
...
- Pediram **"continue"**, "onde paramos", o andamento de uma demanda ou a lista do que está aberto? → `skills/estado-do-ciclo`. Reconcilie o registro com os artefatos em disco antes de gerar qualquer coisa nova — e um gate registrado como aprovado nunca substitui a aprovação do usuário nesta sessão.
